...
class TestMediaAndMetaStorageDirectly(Control_meta_test_case):
    ...
    def test_create_media_only_delete_roundtrip(self):
        ...
        metadata = mms.create_new_media_entry("text/plain", "foo", user)
    ...
    def test_create_meta_then_data(self):
        mms = RDBMSMediaAndMetaStorage("")
        return_metadata=mms.create_new_media_entry('text/plain', 'foo', self.get_dummy_user())
        ...
        mms.post_media_to_id(contentid, "text/plain", "zor", self.get_dummy_user())

    def test_push_meta_for_specific_nonexistant_id(self):
        mms = RDBMSMediaAndMetaStorage("")
        try:
            result = mms.post_media_to_id("1", "text/plain", "this is plain text", self.get_dummy_user())
